...
public class TaskbarController extends UIController {
    ...
    private void drawTaskbar(UIHost host) {
        ...
        try {
            if(button instanceof Button) ((Button) button).setTypeface(Typeface.createFromFile("/system/fonts/Roboto-Regular.ttf"));
        ...
        if(TaskbarPosition.isVertical(context)) {
            // Hide dashboard button - not needed for Xiaomi sidebar style
            if(dashboardButton != null) dashboardButton.setVisibility(View.GONE);
            // Remove old circle if exists (handles rotation)
            if(draggableCircle != null) {
                try { ((WindowManager) context.getSystemService(Context.WINDOW_SERVICE)).removeView(draggableCircle); } catch(Exception ignored) {}
                draggableCircle = null;
            }
            if(buttonLayout != null) {
                addDraggableCircle(host, context);
        ...
        GradientDrawable taskbarBg = new GradientDrawable();
        taskbarBg.setShape(GradientDrawable.RECTANGLE);
        taskbarBg.setCornerRadius(32f);
        taskbarBg.setColor(backgroundTint);
        taskbarBg.setStroke(3, 0x33FFFFFF);
        layout.setBackground(taskbarBg);
        layout.setClipToOutline(true);
        layout.findViewById(R.id.divider).setBackgroundColor(
                pref.getBoolean(PREF_CENTERED_ICONS, false) ? 0 : accentColor
        );
        if(button instanceof Button) ((Button) button).setTextColor(accentColor);

        applyMarginFix(host, layout, params);

        if(isFirstStart && TaskbarPosition.isVertical(context)) {
            startButton.setVisibility(View.GONE);
            space.setVisibility(View.GONE);
        } else if(isFirstStart && FreeformHackHelper.getInstance().isInFreeformWorkspace())
            showTaskbar(false);
        ...
        updateParamsRunnable = () -> {
            ...
            if(TaskbarPosition.isVertical(context)) {
                newParams = params.updateHeight(WindowManager.LayoutParams.WRAP_CONTENT);
            } else {
    ...
    void calculateScrollViewParams(Context context,
        ...
        if(TaskbarPosition.isVertical(context)) {
            ...
            int maxSidebarHeight = (int)(display.height * 0.55f);
            params.height = (int) Math.min(Math.min(maxRecentsSize, maxScreenSize), maxSidebarHeight)
    ...
    private void showTaskbar(boolean clearVariables) {
        ...
        if(startButton.getVisibility() == View.GONE) {
            ...
            if(draggableCircle != null) {
                ...
                try { wm2.removeView(draggableCircle); } catch(Exception ignored) {}
                try { wm2.addView(draggableCircle, draggableCircle.getLayoutParams()); } catch(Exception ignored) {}
            }
    ...
    public void onDestroyHost(UIHost host) {
        ...
        if(draggableCircle != null)
            try {
                ((WindowManager) context.getSystemService(Context.WINDOW_SERVICE)).removeView(draggableCircle);
                draggableCircle = null;
            } catch (IllegalArgumentException ignored) {}
    ...
    private void updateButton(boolean isCollapsed) {
        ...
        if(button != null && button instanceof Button) ((Button) button).setText(context.getString(isCollapsed ? R.string.tb_right_arrow : R.string.tb_left_arrow));
    ...
    private void addDraggableCircle(UIHost host, Context context) {
        ...
        circle.setStroke((int)(1.5f * density), 0x55FFFFFF);

        FrameLayout container = new FrameLayout(context);
        View pill = new View(context);
        pill.setBackground(circle);
        FrameLayout.LayoutParams pillLp = new FrameLayout.LayoutParams(circleSize, circleSize);
        ...
        int savedX = pref.getInt("circle_x", 0);
        int savedY = pref.getInt("circle_y", screenHeight / 2 - circleSize / 2);
        ...
        final int[] lx = {0}, ly = {0}, ox = {0}, oy = {0};
        final boolean[] moved = {false};

        container.setOnTouchListener((v, event) -> {
            switch(event.getActionMasked()) {
                case MotionEvent.ACTION_DOWN:
                    lx[0] = ox[0] = (int) event.getRawX();
                    ly[0] = oy[0] = (int) event.getRawY();
                    moved[0] = false;
                    return true;
                case MotionEvent.ACTION_MOVE:
                    if(Math.abs((int)event.getRawX()-ox[0]) + Math.abs((int)event.getRawY()-oy[0]) > 8)
                        moved[0] = true;
                    if(moved[0]) {
                        p.x = Math.max(0, Math.min(screenWidth - circleSize, p.x + (int)event.getRawX() - lx[0]));
                        p.y = Math.max(0, Math.min(screenHeight - circleSize, p.y + (int)event.getRawY() - ly[0]));
                        lx[0] = (int) event.getRawX();
                        ly[0] = (int) event.getRawY();
                        try { wm.updateViewLayout(container, p); } catch(Exception ignored) {}
                    }
                    return true;
                case MotionEvent.ACTION_UP:
                    if(!moved[0]) toggleTaskbar(true);
                    else pref.edit().putInt("circle_x", p.x).putInt("circle_y", p.y).apply();
                    return true;
            }
            return false;
        });

        draggableCircle = container;
        try { wm.addView(container, p); } catch(Exception ignored) {}
    }
